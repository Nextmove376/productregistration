      ...
      {steps.map((s, i) => (
        <div key={i} className="relative flex gap-6 pb-8 last:pb-0">
          ...
          <div className="flex-1">
            {/* The text column is only ~245px at 375px (a 40px badge plus `gap-6` eat
                the rest), so title and timeline pill could not share one line. Wrap
                instead of squeezing, and keep the pill whole. */}
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <h3 className="font-serif text-lg">{s.title}</h3>
              <span className="shrink-0 rounded-full bg-[var(--cream)] px-3 py-0.5 text-xs text-muted-foreground">{s.timeline}</span>
            </div>
