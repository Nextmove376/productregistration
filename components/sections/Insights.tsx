...
const POSTS = [
  ...
  { tag: 'Comparison', title: 'Set Up a Business in Dubai: Mainland vs Free Zone vs Offshore â€” Complete 2026 Guide', read: '15 min read', slug: 'how-to-set-up-a-business-in-dubai-uae' },
];
    ...
    <section className="bg-sand/60 py-20">
      <div className="mx-auto max-w-6xl px-5">
        <div className="mb-12 flex flex-wrap items-end justify-between gap-4">
          ...
          <Link href="/blog" className="text-sm font-semibold text-primary hover:underline">More insights â†’</Link>
        </div>
        <div className="divide-y divide-border border-y border-border">
          {POSTS.map((p, i) => (
            <Link key={i} href={`/blog/${p.slug}`} className="group grid grid-cols-12 items-center gap-4 py-6 transition-colors hover:bg-card sm:gap-6">
              ...
              <span className="col-span-1 hidden text-right text-lg transition-transform group-hover:-translate-y-1 group-hover:translate-x-1 sm:block">â†—</span>
            </Link>
          ))}
        </div>
      </div>
