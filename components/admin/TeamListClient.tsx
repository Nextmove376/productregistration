...
export default function TeamListClient() {
  ...
  /**
   * `/api/team` returns the full admin shape (including `is_active` and inactive
   * rows) when a session is present, and a reduced public shape otherwise.
   */
  const load = useCallback(async () => {
    ...
    try {
      ...
      addToast('error', err instanceof ApiError ? err.message : 'Failed to load team members');
    } finally {
      setLoading(false);
    }
  }, [addToast]);
  ...
  const handleDelete = async (member: TeamMember) => {
    if (!confirm(`Move "${member.name}" to trash? You can restore them later.`)) return;

    setDeletingId(member.id);
    try {
      await api.delete(`/api/team/${member.id}`);
      setMembers((prev) => prev.filter((m) => m.id !== member.id));
      addToast('success', `"${member.name}" moved to trash`);
    } catch (err) {
    ...
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">{members.length} member(s)</p>
        </div>
        <Link
          href="/admin/team/new"
              ...
              <tbody className="divide-y divide-gray-100">
                {members.map((m) => (
                  <tr key={m.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">{m.sort_order}</td>
                    <td className="px-6 py-4">
