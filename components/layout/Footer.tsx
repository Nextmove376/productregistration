    ...
    <footer className="border-t border-ink-foreground/10 bg-ink text-ink-foreground/70">
      <div className="mx-auto grid max-w-6xl gap-10 px-5 py-14 md:grid-cols-4">
        ...
        {[
          ...
          { h: 'Contact', l: [{ label: '+971 52 910 2088', href: 'tel:+971529102088' }, { label: 'hello@nextmoveservices.ae', href: 'mailto:hello@nextmoveservices.ae' }, { label: 'Dubai, UAE', href: '/contact' }, { label: 'WhatsApp us', href: 'https://wa.me/971529102088', external: true }] },
        ].map((c) => (
          <div key={c.h}>
            ...
            <ul className="mt-4 space-y-2 text-sm">
              {c.l.map((x) => (
                <li key={x.label}>
                  {x.external || x.href.startsWith('tel:') || x.href.startsWith('mailto:') ? (
                    <a
                      href={x.href}
                      {...(x.external ? { target: '_blank', rel: 'noopener noreferrer' } : {})}
                      className="transition hover:text-ink-foreground"
                    >
                      {x.label}
                    </a>
                  ) : (
                    <Link href={x.href} className="transition hover:text-ink-foreground">
                      {x.label}
                    </Link>
                  )}
                </li>
