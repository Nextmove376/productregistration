...
export async function POST(request: NextRequest) {
  ...
  const [result] = await pool.execute(
    `INSERT INTO posts (title, slug, excerpt, content, featured_image, image_alt, category_id, author, status, published_at, meta_title, meta_description, og_image, reading_minutes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [d.title, d.slug, d.excerpt, d.content, d.featured_image || null, d.image_alt, d.category_id, d.author, d.status, d.published_at || null, d.meta_title, d.meta_description, d.og_image || null, d.reading_minutes]
  );
