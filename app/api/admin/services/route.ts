...
export async function POST(request: NextRequest) {
  ...
  const [result] = await pool.execute(
    ...
    [d.title, d.slug, d.tag ?? "", d.summary ?? "", d.body ? JSON.stringify(d.body) : null, d.icon ?? "", d.hero_image ?? "", d.sort_order ?? 0, d.is_active ?? 1, d.meta_title ?? "", d.meta_description ?? "", d.og_image ?? ""]
  );
