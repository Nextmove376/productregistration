...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    servicePages = (services as any[]).map((s) => ({ url: `${base}/services/${s.slug}`, lastModified: new Date(s.updated_at), changeFrequency: 'monthly' as const, priority: 0.7 }));
    const [posts] = await pool.execute("SELECT slug, updated_at FROM posts WHERE status = 'published'");
    blogPages = (posts as any[]).map((p) => ({ url: `${base}/blog/${p.slug}`, lastModified: new Date(p.updated_at), changeFrequency: 'monthly' as const, priority: 0.6 }));
  } catch {}
